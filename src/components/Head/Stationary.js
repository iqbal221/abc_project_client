...
import Read from "../../APIServices/ReadStationaryAPI";
import Delete from "../../APIServices/DeleteAPI";
...
function Stationary() {
  ...
  const DeleteItem = (UserID) => {
    Delete(UserID).then((result) => {
      if (result === true) {
        alert("Do you want to Delete this item?");
        // navigate('/')
        window.location.reload(false);
      } else {
        alert("Please correct command for delete");
        navigate("/");
      }
    });
  };
